...
def _remap_q8_attention_state_dict(state_dict: dict[str, torch.Tensor]) -> dict[str, torch.Tensor]:
    in_proj_pattern = re.compile(r"^(.*\.self_attn)\.in_projs\.(\d+)\.(weight(?:_scb)?)$")
    out_proj_pattern = re.compile(r"^(.*\.self_attn)\.out_projs\.(\d+)\.(weight(?:_scb)?)$")
    grouped_tensors: dict[tuple[str, str, str], list[tuple[int, torch.Tensor]]] = {}
    remapped_state_dict: dict[str, torch.Tensor] = {}

    for key, tensor in state_dict.items():
        match = in_proj_pattern.match(key)
        if match is not None:
            prefix, index, suffix = match.groups()
            grouped_tensors.setdefault((prefix, "in_proj_weight", suffix), []).append((int(index), tensor))
            continue

        match = out_proj_pattern.match(key)
        if match is not None:
            prefix, index, suffix = match.groups()
            grouped_tensors.setdefault((prefix, "out_proj", suffix), []).append((int(index), tensor))
            continue

        remapped_state_dict[key] = tensor

    for (prefix, target_name, suffix), indexed_tensors in grouped_tensors.items():
        indexed_tensors.sort(key=lambda item: item[0])
        merged_tensor = torch.cat([tensor for _, tensor in indexed_tensors], dim=0)
        if target_name == "in_proj_weight":
            target_key = f"{prefix}.{target_name}"
        else:
            target_key = f"{prefix}.{target_name}.{suffix}"
        if suffix == "weight_scb" and target_name == "in_proj_weight":
            target_key = f"{prefix}.{target_name}_scb"
        remapped_state_dict[target_key] = merged_tensor

    return remapped_state_dict


def _load_safetensors_model(model: torch.nn.Module, filename: Path | str, strict: bool = True) -> None:
    try:
        load_model(model, filename, strict=strict)
    except RuntimeError as error:
        if "in_projs" not in str(error) and "out_projs" not in str(error):
            raise
        state_dict = load_file(filename)
        remapped_state_dict = _remap_q8_attention_state_dict(state_dict)
...
def get_moshi_lm(filename: str | Path,
    ...
    model.eval()
    if _is_safetensors(filename):
        _load_safetensors_model(model, filename, strict=strict)
    else:
        pkg = torch.load(
            filename,
            "cpu",
